...
constexpr auto SafeHash::New(u64 hash) -> SafeHash {
    return SafeHash {
        ((HashUint)1 << ((sizeof(HashUint) * 8) - 1)) | static_cast<HashUint>(hash)
    };
}

template<typename K, typename V>
auto calculate_layout(usize capacity) -> Result<std::pair<alloc::Layout, usize>, LayoutErr> {
    auto hashes = RXX_RESULT_TRY(alloc::Layout::array<HashUint>(capacity));
    auto pairs = RXX_RESULT_TRY(alloc::Layout::array<std::pair<K, V>>(capacity));
    return hashes.extend(pairs).map([&hashes](std::pair<alloc::Layout, usize> result) -> std::pair<alloc::Layout, usize> {
...
inline auto HashMap<K, V, S>::New() -> HashMap<K, V, hash_map::RandomState> {
    return HashMap<K, V, hash_map::RandomState> {
        nullptr, nullptr, (std::uint64_t)0 - 1, 0, hash_map::RandomState::New()
    };
}
...
inline auto HashMap<K, V, S>::lookup_bucket(K const& key) -> Option<std::pair<K, V>&> {
    ...
    auto dist = usize{0};
    while (dist < cap) {
        auto& bucket = m_buckets[i.as_size_t()];

        if (m_hashes[i.as_size_t()] == 0) {
            return None;
        }
        ...
        if (m_hashes[i.as_size_t()] == hash && bucket.first == key) {
            return Some(bucket);
        }
...
inline void HashMap<K, V, S>::internal_rehash(usize new_capacity) {
    ...
    auto hashes = reinterpret_cast<hash_map::impl::HashUint *>(alloc::Global.alloc(layout));
    auto buckets = reinterpret_cast<std::pair<K, V>*>(hashes + pairs_offset.as_size_t());

    auto new_map = HashMap<K, V, S> {
        hashes,
        buckets,
        new_capacity.wrapping_sub(1),
        0,
...
inline auto HashMap<K, V, S>::entry(K&& k) -> hash_map::Entry<K, V> {
    ...
    auto capacity = m_mask + 1;
    auto i = hash & m_mask;
    auto distance = usize{0};

    std::pair<K, V>* ret_pair = nullptr;
    auto tmp_value = Option<V> { None };

    while (distance < capacity) {
        auto& bucket = m_buckets[i.as_size_t()];

        if (m_hashes[i.as_size_t()] == 0) {
            m_hashes[i.as_size_t()] = hash;
            m_len += 1;
        ...
        if (m_hashes[i.as_size_t()] == hash && bucket.first == k) {
        ...
        auto bucket_dist = bucket_distance(i);
        if (bucket_dist < distance) {
            ...
            std::swap(m_hashes[i.as_size_t()], hash);
...
inline auto HashMap<K, V, S>::insert(K&& k, V&& v) -> Option<V> {
    auto e = entry(std::forward<K>(k));

    if (e.is_occupied())
        return None;

    return Some(e.insert(std::forward<V>(v)));
}
