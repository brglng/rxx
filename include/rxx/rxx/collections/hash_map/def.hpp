...
public:
    ...
    static constexpr auto New(std::uint64_t hash) -> SafeHash {
        return SafeHash {
...
template<typename K, typename V, typename S = RandomState>
class HashMap {
...
public:
    HashMap(std::size_t *hashes, std::pair<K, V> *buckets,
            std::size_t mask, std::size_t len, S&& hash_builder) :
        m_hashes { hashes },
        m_buckets { buckets },
        m_mask { mask },
        m_len { len },
    ...
    static auto New() -> HashMap<K, V, RandomState>;

private:
    auto bucket_distance(std::size_t i) const -> std::size_t {
        return (i + m_mask + 1 - (m_hashes[i] & m_mask)) & m_mask;
    }
